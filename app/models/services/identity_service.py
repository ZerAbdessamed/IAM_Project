...
from app.models.services.auth_service import is_password_reused, store_password_history
...
def create_identity(data):
    ...
    store_password_history('user', user.id, user.password_hash)
...
def update_identity(user, data):
    ...
    raw_password = common.pop("password")

    if is_password_reused('user', user.id, raw_password, current_password_hash=user.password_hash):
        raise IdentityValidationError("New password cannot reuse recent password history")
